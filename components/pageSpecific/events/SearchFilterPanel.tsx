...
import { FilterCalendar } from './panelFilters/ByDate';
...
const SearchFilterPanel = connector(({ replaceContent, filters }: SearchFilterPanelProps) => {
    const [initialFilterState, setIsInitialFilterState] = useState(true)
    useEffect(() => {
        setIsInitialFilterState(isInitialFilterState())
    }, [filters])
    ...
    return (
        <div className={"searchPanel relative h-full elevate-200 text-primary-content rounded-tr-xl rounded-br-xl grid grid-rows-[auto_1fr]"}>
            ...
            <div className={"h-full"}>
                ...
                {!replaceContent && <SearchPanelFilters />}
            </div>
            ...
            <a role="button" onClick={clearEventFilter} className={clsx("bg-primary text-primary-content w-[calc(100%-1rem)] ml-2 mb-2 rounded-lg text-center py-2 px-1", initialFilterState && "hidden")}>Search</a>
        </div>
    )
})
