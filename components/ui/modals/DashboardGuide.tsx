...
import { hideModal } from '#/state/slices/ui';

const connector = connect((state: RootState, props: any) => ({
    open: state.UI.modals.dashboardGuide,
    props: props
}))
...
/* TODO: Actually come back and implement the different dashboard features here. This is a primo place for that carousel package.*/

const DashboardGuideModal = connector(({ open }: DashboardGuideModalProps) => {
    const closeModal = () => {
        store.dispatch(hideModal("dashboardGuide"))
    }

    return (
        <div
            className={clsx("fixed inset-0 z-10 overflow-y-auto scale-0 opacity-0 transition-all duration-300", open && "opacity-100 sm:scale-100")}
        ...
        >
            <div className="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div className="relative inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl dark:bg-gray-900 sm:my-8 sm:align-middle sm:max-w-md sm:w-full sm:p-6">
                    <div>
                        <Image className="object-cover w-full h-48 rounded-md" src="https://images.unsplash.com/photo-1579226905180-636b76d96082?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=687&q=80" alt="">
...
DashboardGuideModal.displayName = "DashboardGuideModal"
